...
import { getSaleDocuments, deleteDocument } from "@/app/actions/ventes.actions";
import {
  Table,
  TableBody,
  TableCell,
...
import {
  Plus, Search, Printer, Trash2, RefreshCw, MoreVertical, Eye, Truck, Edit
} from "lucide-react";
...
export default function DeliveryNoteListPage() {
  ...
  const loadDocs = async () => {
    setIsLoading(true);
    const res = await getSaleDocuments("DELIVERY");
    if (res.success) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Supprimer ce Bon d'Enlèvement ? Cela restaurera le stock !")) return;
    ...
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-slate-900/50">
      <div className="p-6 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-orange-600 shadow-sm">
            <Truck size={28} />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Bons d'Enlèvement</h1>
            <p className="text-slate-500 text-sm">
               {filteredDocs.length} {filteredDocs.length > 1 ? 'documents trouvés' : 'document trouvé'} sur {docs.length}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={loadDocs} disabled={isLoading}>
            <RefreshCw className={cn("h-4 w-4 mr-2", isLoading && "animate-spin")} />
            Actualiser
          </Button>
          <Link href="/ventes/enlevement/create">
            <Button size="sm" className="bg-orange-600 hover:bg-orange-700 shadow-sm text-white font-bold">
      ...
      <div className="flex-1 px-6 pb-6 overflow-hidden">
        <div className="h-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-auto">
          <Table>
            <TableHeader className="bg-slate-50/80 dark:bg-slate-900/50 sticky top-0 z-10">
              <TableRow>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={6} className="text-center py-20 text-slate-400">Chargement...</TableCell></TableRow>
              ) : filteredDocs.length === 0 ? (
                <TableRow><TableCell colSpan={6} className="text-center py-20 text-slate-400 italic">Aucun document trouvé</TableCell></TableRow>
              ) : (
                filteredDocs.map((doc) => (
                  <TableRow key={doc.id} className="group hover:bg-orange-50/20 dark:hover:bg-orange-900/10 transition-colors">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56">
                          <DropdownMenuGroup>
                            ...
                            <DropdownMenuItem onClick={() => router.push(`/ventes/enlevement/edit/${doc.id}`)}>
